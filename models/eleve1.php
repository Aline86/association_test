<?php
 require_once 'connection.php';
  class Eleve {
    ...
    public static function listeNiveauId($id){
        $niveaux = [];
        $db = Db::getInstance();       
        $q = $db->query('SELECT * FROM eleve WHERE niveau_1="'.$id.'" OR niveau_2="'.$id.'"');
        $count=$q->rowCount();
        if($count>=1){
            while ($donnees = $q->fetch(PDO::FETCH_ASSOC))
            {
    ...
    public static function update_niveau1($niveau1, $id)
    { 
        $db = Db::getInstance();
        
        $requete='UPDATE eleve SET niveau_1="'.$niveau1.'" WHERE id_eleve="'.$id.'"';
        $query=$db->query($requete);
    
    }
    public static function update_niveau2($niveau2, $id)
    { 
        $db = Db::getInstance();
        
        $requete='UPDATE eleve SET niveau_2="'.$niveau2.'" WHERE id_eleve="'.$id.'"';
        $query=$db->query($requete);
    
    }
    public function get_niveau_eleve_1($id){
        $db = Db::getInstance();
        $q = $db->query('SELECT libelle_niveau FROM eleve INNER JOIN niveau ON niveau.id_niveau=eleve.niveau_1 WHERE niveau.id_niveau="'.$activation.'"');
