if (typeof sessionStorage != 'undefined') {
    if ('message1' in sessionStorage) {
        document.getElementById('prof').value = sessionStorage.getItem('message1');
    }
} else {
    alert("sessionStorage n'est pas supporté");
}
if (typeof sessionStorage != 'undefined') {
    if ('message2' in sessionStorage) {
        document.getElementById('level').value = sessionStorage.getItem('message2');
    }
} else {
    alert("sessionStorage n'est pas supporté");
}
                    ...
                    document.querySelectorAll(".cell").forEach(elem =>{
                        for(i=0; i<date.length; i++){ 
                            ...
                            if(currentMonth.getMonth()==date[i].getMonth() && elem.innerHTML==date[i].getDate()){
                                ...
                                var date_du_jour=date[i].getDate()
                                ...
                                //Apparemment les valeurs qui passent par date_du_jour sont filtrées par elem.addEventListener, c'est pourquoi la date du jour sélectionnée correspond bien à la date du jour survolée
                                    elem.addEventListener('mouseenter', function(e){
                                        e.preventDefault();
                                            ...
                                            if(elem.innerHTML=="" || elem.innerHTML.includes('h')){
                                                console.log(date_du_jour)
                                                elem.innerHTML=date_du_jour
                                            }else{
                                                ...
                                                console.log(2)
                                                    ...
                                                    xhttp.onreadystatechange = function () {
                                                        if (this.readyState == 4 && this.status == 200) {
                                                            ...
                                                            elem.innerHTML=""
                                                            for(var i in myArr) {
                                                                elem.innerHTML+=' '+myArr[i].horaire
                                                            } 
